...
async function loadShipperDashboard() {
  ...
  if (table) {
    if (recentShipments.length > 0) {
      ...
      table.innerHTML = '<tr><td colspan="4">لا يوجد رحلات</td></tr>';
    }
  }
...
async function loadAvailableCarriers() {
  ...
  try {
    // الحصول على آخر رحلة نشطة للمستخدم
    ...
    if (!shipments || shipments.length === 0) {
      document.getElementById('availableCarriersTable').innerHTML = '<tr><td colspan="6">لا توجد رحلات نشطة للبحث عن ناقلين</td></tr>';
      return;
    }
