...
class EnhancedMessagingSystem {
    ...
    async createNotificationForMessage(messageData) {
        const notificationData = {
            ...
            type: messageData.message_type === 'contact_request' ? 'match' : 'message',
            title: messageData.message_type === 'contact_request' ? 'طلب تواصل جديد!' : 'رسالة جديدة',
            message: `من: ${this.currentUser.full_name || 'مستخدم'}\nالموضوع: ${messageData.subject}`,
            priority: messageData.priority || 'normal',
            related_id: messageData.id,
            related_type: 'message',
            action_url: messageData.message_type === 'contact_request' 
                ? `/messages.html?type=contact_requests&id=${messageData.id}` 
                : `/messages.html?id=${messageData.id}`,
            action_text: 'عرض الرسالة',
            icon: messageData.message_type === 'contact_request' ? 'fas fa-handshake' : 'fas fa-envelope'
        };
