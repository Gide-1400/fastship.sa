...
class FastShipMatcherV2 {
  ...
  async findAndSaveMatches(shipment, forceRecalculate = false) {
    try {
      ...
      for (const trip of trips) {
        ...
        // حفظ فقط المطابقات الجيدة (أكثر من 5%)
        if (matchScore >= 5) {
  ...
  calculateRouteScore(shipment, trip) {
    ...
    // يجب أن تتطابق النقطتين معاً
    const routeScore = (pickupMatch + deliveryMatch) / 2;
    
    // إذا إحدى النقاط لا تتطابق أبداً، النتيجة منخفضة جداً
    if (pickupMatch === 0 || deliveryMatch === 0) {
      console.log(`    ❌ المسار لا يتطابق - إحدى النقاط مختلفة تماماً`);
      return 0.1;
    }
