...
class SessionManager {
    ...
    async _doInit() {
        try {
            ...
            await this._waitForSupabaseReady();
            
            // Give more time for session restoration
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
                ...
                if (session) {
                    ...
                    // Store session backup immediately when found
                    const userInfo = {
    ...
    async _waitForSupabaseReady() {
        await this._waitForSupabase();
        
        // Additional check to ensure Supabase auth is ready
        let attempts = 0;
        while (attempts < 10) {
            try {
                const { data } = await window.supabaseClient.auth.getSession();
                // If we can get session data (even if null), auth is ready
                console.log('✅ Supabase auth ready');
                break;
            } catch (error) {
                console.log(`⏳ Waiting for Supabase auth... attempt ${attempts + 1}`);
    ...
    async _loadUserProfile(authUser) {
        try {
            ...
            try {
                ...
                console.log('Could not get user metadata:', e);
    ...
    getUserType() {
        ...
        // Try to get from current user metadata
        if (this.currentUser?.metadata?.account_type) {
            return this.currentUser.metadata.account_type;
        }
        if (this.currentUser?.metadata?.user_type) {
            return this.currentUser.metadata.user_type;
        }

        // Try to get from Supabase auth user metadata directly
        if (window.supabaseClient) {
            try {
                const { data: { user } } = window.supabaseClient.auth.getUser();
        ...
        try {
            ...
            if (userData) {
                ...
                return user.user_metadata?.user_type || user.user_metadata?.account_type || null;
            }
        } catch (e) {}

        // Last fallback: since this is carrier-app, assume carrier
        if (window.location.pathname.includes('carrier-app')) {
            return 'carrier';
        }
    ...
    async goToDashboard() {
        ...
        if (!this.currentUser?.profile) {
            ...
            try {
                ...
                // إنشاء سجل في جدول carriers أو shippers حسب نوع المستخدم
                if (userType === 'carrier') {
                    ...
                    if (carrierError) {
                        ...
                        console.log('Carrier record created successfully for user:', userData.id);
                    }
        ...
        if (userType === 'carrier') {
            ...
            // توجيه الشاحن إلى قسم الشاحنين
            window.location.href = '../shipper-app/shipper-home.html';
        } else {
    ...
    _getHomePath() {
        // من مجلد carrier-app، نحتاج للعودة للمجلد الأساسي
        return '../index.html';
    }

    _getAuthPath(page) {
        // بما أننا في مجلد carrier-app، نوجه للمسار النسبي
        return page;
    }

    _getDashboardPath(page) {
        // بما أننا في قسم الشاحن فقط، نوجه دائماً للمسار النسبي
        return page;
    }
    ...
    async createNotification(userId, type, title, message, relatedId = null) {
        try {
            ...
            if (!userId || typeof userId !== 'string' || userId.trim() === '') {
                console.error('❌ createNotification: userId غير صحيح:', userId);
                return null;
            }
    ...
    // Create a trip notification for current user
    async createTripNotification(title, message, tripId) {
        if (!this.currentUser) return null;
        return this.createNotification(this.currentUser.id, 'trip', title, message, tripId);
    }
    ...
    // Ensure carrier record exists for current user
    async ensureCarrierRecord() {
        ...
        try {
            // Check if carrier record exists
            const userId = this.currentUser.profile?.id || this.currentUser.id;
            const { data: carrierData, error: carrierError } = await window.supabaseClient
                .from('carriers')
                .select('id')
                .eq('user_id', userId)
                .single();
            ...
            if (carrierData) {
                return carrierData.id; // Carrier record exists
            }

            // If not found (PGRST116 error), create it
            if (carrierError && carrierError.code === 'PGRST116') {
                console.log('Creating carrier record for user...');
                
                const { data: newCarrierData, error: createError } = await window.supabaseClient
                    .from('carriers')
                    .insert([{
                        user_id: userId,
                        vehicle_type: 'any',
                ...
                if (createError) {
                    console.error('Error creating carrier record:', createError);
                    throw new Error('فشل في إنشاء سجل الشاحن');
                }

                console.log('Carrier record created successfully');
                return newCarrierData.id;
            }
            ...
            console.error('Error ensuring carrier record:', error);
            throw error;
        }
    }
}
...
document.addEventListener('visibilitychange', function() {
    if (!document.hidden && window.sessionManager.isInitialized && !window.sessionManager.isReinitializing) {
        // Check if session is still valid when user returns to tab
        window.sessionManager.isReinitializing = true;
        setTimeout(() => {
            window.sessionManager._doInit().finally(() => {
                window.sessionManager.isReinitializing = false;
            });
        }, 1000); // زيادة التأخير لتجنب الإعادة السريعة
    }
});

// Check session on page focus
window.addEventListener('focus', function() {
    if (window.sessionManager.isInitialized && !window.sessionManager.isReinitializing) {
        window.sessionManager.isReinitializing = true;
        setTimeout(() => {
            window.sessionManager._doInit().finally(() => {
                window.sessionManager.isReinitializing = false;
