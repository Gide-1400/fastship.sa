...
class SessionManager {
    ...
    getUserType() {
        ...
        // Last fallback: since this is shipper-app, assume shipper
        if (window.location.pathname.includes('shipper-app')) {
            return 'shipper';
        }
    ...
    async goToDashboard() {
        ...
        if (userType === 'shipper') {
            ...
            // توجيه الناقل إلى قسم الناقلين
            window.location.href = '../carrier-app/carrier-home.html';
        } else {
    ...
    async ensureShipperRecord() {
        if (!this.isLoggedIn()) {
            throw new Error('المستخدم غير مسجل دخول');
        }

        try {
            // تأكد من توفر ملف المستخدم قبل معالجة سجل الشاحن
            let profileId = this.currentUser.profile?.id;
            if (!profileId) {
                const { data: freshProfile, error: profileError } = await window.supabaseClient
                    .from('users')
                ...
                if (profileError) {
                    console.error('Error loading profile before ensuring shipper:', profileError);
                    throw profileError;
                }

                if (!freshProfile) {
                    throw new Error('تعذر العثور على ملف المستخدم لإنشاء سجل الشاحن');
                }
            ...
            // التحقق من وجود سجل الشاحن الحالي
            ...
            if (shipperError) {
                console.error('Error checking existing shipper record:', shipperError);
                throw shipperError;
            }
            ...
            const { data: newShipperData, error: createError } = await window.supabaseClient
                ...
                .upsert([
                    {
                        user_id: profileId,
                        company_name: this.currentUser.profile?.full_name || this.currentUser.email || 'شاحن',
                        status: 'active'
                    }
                ], { onConflict: 'user_id' })
                .select('id')
